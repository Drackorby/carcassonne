...
if __name__ == '__main__':
    ...
    print("nº states ", len(states))
    print("nº actions ", len(actions))
    for i in range(8):
        ...
        def __init__(self, observation_space: spaces.Dict):
            ...
            for key, subspace in observation_space.spaces.items():
                ...
                if key == "image":
                    ...
                    extractors[key] = nn.Sequential(nn.Conv2d(subspace.shape[0], 64, 5, stride=1, padding="same"), nn.BatchNorm2d(64), nn.Conv2d(64, 128,3, stride=1, padding="same"), nn.BatchNorm2d(128), nn.MaxPool2d(2), nn.Conv2d(128, 256,3, stride=1, padding="same"), nn.BatchNorm2d(256), nn.MaxPool2d(2), nn.Conv2d(256, 512,3, stride=1, padding="same"),
                                                    nn.Flatten())  
                    # print(key)
                    # summary(extractors[key], (1,) + subspace.shape)
                    total_concat_size += 128
                    ...
                    extractors[key] = nn.Sequential(nn.Conv2d(subspace.shape[0], 64, 5, stride=1, padding="same"), nn.BatchNorm2d(64), nn.Conv2d(64, 128,3, stride=1, padding="same"), nn.BatchNorm2d(128), nn.MaxPool2d(2), nn.Conv2d(128, 256,3, stride=1, padding="same"), nn.BatchNorm2d(256), nn.MaxPool2d(2), nn.Conv2d(256, 256,3, stride=1, padding="same"),
                                                    nn.Flatten())  
                    # print(key)
                    # summary(extractors[key], (1,) + subspace.shape)
                    total_concat_size += 128
                    ...
                    extractors[key] = nn.Sequential(nn.Conv2d(subspace.shape[0], 64, 5, stride=1, padding="same"), nn.BatchNorm2d(64), nn.Conv2d(64, 128,3, stride=1, padding="same"), nn.BatchNorm2d(128), nn.MaxPool2d(2), nn.Conv2d(128, 128,3, stride=1, padding="same"), nn.BatchNorm2d(128), nn.MaxPool2d(2), 
                                                    nn.Flatten()) 
                    # print(key, subspace.shape)
                    # summary(extractors[key], (1,) + subspace.shape)
    ...
    policy_kwargs = dict(
        ...
        net_arch=[
            dict(pi=[8192, 8192], vf=[8192, 4096, 1024, 256, 128], dropout=0.5, activation_fn=nn.LeakyReLU)
        ]
    )
    ...
    train_size = int(0.5 * len(expert_dataset))
    ...
    a2c_student = PPO("MultiInputPolicy", env, verbose=1,tensorboard_log="runs/", device="cuda", n_steps=1024, ent_coef=0.02, learning_rate=linear_schedule(0.0003), policy_kwargs=policy_kwargs)
    # a2c_student = PPO("MultiInputPolicy", env, verbose=1, policy_kwargs=policy_kwargs)
    ...
    try: 
        ...
        a2c_student.save("pretrained_carcassone2")
    ...
    a2c_student.save("pretrained_carcassone")
